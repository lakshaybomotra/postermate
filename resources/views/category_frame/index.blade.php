      ...
      <div class="card-body">
          <div style="float: left;">
            <select class="form-control" id="category_dropdown" name="category_dropdown" onchange="location = this.value;">
                <option value="{{url('admin/category-frame')}}" @if(empty($name)) selected @endif>Select Category</option>
        ...
        <!-- <div class="dropdown" style="float: left;">
          <button class="btn btn-primary dropdown-toggle dropbtn" onclick="myFunction()" type="button" id="dropdownMenu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            @if(!empty($name)) {{$name}} @else Select Category @endif
          </button>
          <div class="dropdown-menu dropdown-content" aria-labelledby="dropdownMenu" id="myDropdown"> 
            <input type="text" placeholder="Search.." id="myInput" onkeyup="filterFunction()">
              @foreach($category as $c)
              <a class="dropdown-item" href="{{url('admin/category-get/'.$c->id)}}">{{$c->name}}</a>
              @endforeach
          </div>
        </div> -->
        ...
        <div class="row d-flex justify-content-end mb-3">
          <div class="col-md-3 col-xs-12 text-right" style="float: right;">
            ...
            <div class="dropdown" style="float:right">
              ...
              {!! Form::open(['url' => 'admin/category-frame-action','method'=>'POST','class'=>'form-horizontal','id'=>'form1']) !!}
          ...
          <div>
            <a href="{{ route('category-frame.create')}}" class="btn btn-success ml-2">Add New</a>
          </div>
        </div>
          ...
          <div class="col-lg-3 col-sm-6 col-xs-12">
            <div class="block_wallpaper add_wall_category" style="box-shadow:0px 3px 8px rgba(0, 0, 0, 0.3)">
              ...
              <div class="wall_image_title">
                ...
                {!! Form::open(['url' => 'admin/category-frame/'.$frame->id,'method'=>'DELETE','class'=>'form-horizontal','id'=>'form_'.$frame->id]) !!}
...
<script type="text/javascript">
    ...
    $(function(){
      ...
      $(".checkbox2").change(function(){
        ...
        $.ajax({
          type: "POST",
          url: "{{url('admin/category-frame-type')}}",
      ...
      $(".frame-switch").change(function(){
        ...
        $.ajax({
          type: "POST",
          url: "{{url('admin/category-frame-status')}}",
          data: { checked : checked , id : id},
          headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
          success: function(data) {
            new PNotify({
