    ...
    <div class="card card-primary">
      ...
      <div class="card-body">
        <div style="float: left;">
          <select class="form-control" id="sticker_dropdown" name="sticker_dropdown" onchange="location = this.value;">
              <option value="{{url('admin/sticker')}}" @if(empty($name)) selected @endif>Select Sticker Category</option>
              @foreach($category as $c)
                  <option value="{{url('admin/sticker-category-get/'.$c->id)}}" @if(!empty($name) && $name == $c->name) selected @endif>{{$c->name}}</option>
              @endforeach
          </select>
        </div>
        ...
        <!-- <div class="dropdown" style="float: left;">
          <button class="btn btn-primary dropdown-toggle dropbtn" onclick="myFunction()" type="button" id="dropdownMenu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            @if(!empty($name)) {{$name}} @else Select  Sticker Category @endif
          </button>
          <div class="dropdown-menu dropdown-content" aria-labelledby="dropdownMenu" id="myDropdown"> 
            <input type="text" placeholder="Search.." id="myInput" onkeyup="filterFunction()">
              @foreach($category as $c)
              <a class="dropdown-item" href="{{url('admin/sticker-category-get/'.$c->id)}}">{{$c->name}}</a>
              @endforeach
          </div>
        </div> -->
        ...
        <div class="row d-flex justify-content-end mb-3">
          ...
          <div>
            <a href="{{ route('sticker.create')}}" class="btn btn-success ml-2">Add New</a>
          </div>
        </div>
          ...
          <div class="col-lg-3 col-sm-6 col-xs-12">
            ...
            <div class="wall_category_block" style="z-index: 1">
                <div class="checkbox" style="float: right;z-index: 1">
                  <input type="checkbox" name="post_ids[]" id="checkbox0" value="{{$sticker->id}}" class="post_ids mt-1" style="width: 16px;height: 16px;">
                </div>
              </div>
              <div class="wall_image_title">
                <h2 style="font-size: 20px">
                  {{$sticker->sticker_category->name}}               
                </h2>
                <ul>
                  <li><a href="{{url('admin/sticker/'.$sticker->id.'/edit')}}" data-toggle="tooltip" data-tooltip="Edit"><i class="fa fa-edit"></i></a></li>
                  <li><a href="#" data-id="{{$sticker->id}}" class="btn_delete_a" data-toggle="modal" data-target="#myModal"><i class="fa fa-trash"></i></a></li>
                  <li>
                    <label class="cl-switch cl-switch-red">
                      <input type="checkbox" class="frame-switch" data-id="{{$sticker->id}}" value="1" @if($sticker->status==1) checked @endif>
              ...
              <span>
                <img src="@if(App\Models\StorageSetting::getStorageSetting('storage') == 'DigitalOcean'){{\Storage::disk('spaces')->url('uploads/'.$sticker->image)}} @else {{asset('uploads/'.$sticker->image)}} @endif"/>
              </span>
            </div>
          </div>
          @endforeach
