    ...
    <div class="card card-primary">
      ...
      <div class="card-body">
        <div style="float: left;">
          <select class="form-control" id="custom_post_dropdown" name="custom_post_dropdown" onchange="location = this.value;">
              <option value="{{url('admin/custom-post-frame')}}" @if(empty($name)) selected @endif>Select Custom Category</option>
              @foreach($customPost as $c)
                  <option value="{{url('admin/custom-post-get/'.$c->id)}}" @if(!empty($name) && $name == $c->name) selected @endif>{{$c->name}}</option>
              @endforeach
          </select>
        </div>
        ...
        <!-- <div class="dropdown" style="float: left;">
          <button class="btn btn-primary dropdown-toggle dropbtn" onclick="myFunction()" type="button" id="dropdownMenu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            @if(!empty($name)) {{$name}} @else Select Custom Category @endif
          </button>
          <div class="dropdown-menu dropdown-content" aria-labelledby="dropdownMenu" id="myDropdown"> 
            <input type="text" placeholder="Search.." id="myInput" onkeyup="filterFunction()">
              @foreach($customPost as $c)
              <a class="dropdown-item" href="{{url('admin/custom-post-get/'.$c->id)}}">{{$c->name}}</a>
              @endforeach
          </div>
        </div> -->
        ...
        <div class="row d-flex justify-content-end mb-3">
          <div class="col-md-3 col-xs-12 text-right" style="float: right;">
            ...
            <div class="dropdown" style="float:right">
              ...
              {!! Form::open(['url' => 'admin/custom-post-frame-action','method'=>'POST','class'=>'form-horizontal','id'=>'form1']) !!}
          ...
          <div>
            <a href="{{ route('custom-post-frame.create')}}" class="btn btn-success ml-2">Add New</a>
          </div>
        </div>
...
<script type="text/javascript">
    ...
    $(function(){
      ...
      $(".checkbox2").change(function(){
        ...
        $.ajax({
          type: "POST",
          url: "{{url('admin/custom-post-frame-type')}}",
      ...
      $(".frame-switch").change(function(){
        ...
        $.ajax({
          type: "POST",
          url: "{{url('admin/custom-post-frame-status')}}",
          data: { checked : checked , id : id},
          headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
          success: function(data) {
            new PNotify({
