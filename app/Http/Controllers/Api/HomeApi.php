...
class HomeApi extends Controller
{
    public function getHomeData()
    {
        ...
        $story = Story::where('status',1)->orderBy("id","desc")->take($limit)->get();
        ...
        $festival = Festivals::where('status',1)->orderBy(ApiSetting::getApiSetting("festival_order_type"),ApiSetting::getApiSetting("festival_order_by"))->take($limit)->get();
        $feature = FeaturePost::orderBy('id', 'desc')->take($limit)->get();
        ...
        foreach ($festival as $f) {
            ...
            {
                $video = Video::where("type","festival")->where("festival_id",$f->id)->take($limit)->get();
                $festival_data[] = array(
                    "festivalId" => $f->id,
        ...
        foreach ($business_category as $cat) {
            $video = Video::where("type","business")->where("business_category_id",$cat->id)->take($limit)->get();
        ...
        foreach ($category as $cat) {
            $video = Video::where("type","category")->where("category_id",$cat->id)->take($limit)->get();
            $category_data[] = array(
                "categoryId" => $cat->id,
        ...
        foreach ($feature as $f_data) {
            ...
            if(!empty($festival))
            {
                $video = Video::where("type","festival")->where("festival_id",$festival->id)->take($limit)->get();
                $f_id = $festival->id;
            ...
            if(!empty($category))
            {
                $video = Video::where("type","category")->where("category_id",$category->id)->take($limit)->get();
                $f_id = $category->id;
            ...
            $festival_frame = FestivalsFrame::where("festivals_id",$f_data->festival_id)->where('status',1)->inRandomOrder()->take($limit)->get();
            $category_frame = CategoryFrame::where("category_id",$f_data->category_id)->where('status',1)->inRandomOrder()->take($limit)->get();
